function [x_DC,y_DC, cluster_id] = DC_Clusters(NumDCs,R_DC,numberOfClusters,domainBoundary)
    ...
    x_min = domainBoundary.x_min;
    x_max = domainBoundary.x_max;
    y_min = domainBoundary.y_min;
    y_max = domainBoundary.y_max;
    ...
    while bad_top
        try
            ...
            x_c = x_min + 0.5*(x_length+1);
            y_c = y_min + 0.5*(y_length+1);

            x_t = x_mass;
            y_t = y_mass;
        
            if ((rand < 1.01) && (numberOfClusters == 2))
                theta_rot = (1.0-2.0*(rand < 0.5))*pi/4;
                
                x_mass = x_c+(x_t-x_c)*cos(theta_rot)-(y_t-y_c)*sin(theta_rot);
                y_mass = y_c+(x_t-x_c)*sin(theta_rot)+(y_t-y_c)*cos(theta_rot);
            elseif rand < 0.5
                x_mass = x_c+(x_t-x_c)*cos(pi/2)-(y_t-y_c)*sin(pi/2);
                y_mass = y_c+(x_t-x_c)*sin(pi/2)+(y_t-y_c)*cos(pi/2);
            end
    ...
    p1 = permiate_pi(p1); p2 = permiate_pi(p2); p3 = permiate_pi(p3);
    p4 = permiate_pi(p4); p5 = permiate_pi(p5); p6 = permiate_pi(p6);
    p7 = permiate_pi(p7); p8 = permiate_pi(p8); p9 = permiate_pi(p9);
    ...
    for jj=1:numberOfClusters
        ...
        if (cellsPerCluster_ii > 1)
            ...
            % rndperm = perms([1:length(pc)]);
            % pi = rndperm(randi(length(rndperm)),:);
            % pc = pc(pi',:);
            for ii=1:size(pc,1)
    ...
    % x_c = x_min+2 + 0.5*x_length;
    % y_c = y_min+2 + 0.5*y_length;
    % 
    % x_DC_t = x_DC;
    % y_DC_t = y_DC;
    % if rand < 0.5
    %     y_DC = x_c+(x_DC-x_c)/sqrt(2)-(y_DC-y_c)/sqrt(2);
    %     x_DC = y_c+(x_DC-x_c)/sqrt(2)+(y_DC-y_c)/sqrt(2);
    % end
...
function cell_rings = rings_func(cellsPerCluster)
    if (cellsPerCluster > 0);        cell_rings = 2;    end
    if (cellsPerCluster > 1);        cell_rings = 3;    end
    if (cellsPerCluster > 7);        cell_rings = 4;    end
    if (cellsPerCluster > 19);       cell_rings = 5;    end
...
function pc = permiate_pi(pc)

    pi = randperm(length(pc));
    pc = pc(pi',:);
end
