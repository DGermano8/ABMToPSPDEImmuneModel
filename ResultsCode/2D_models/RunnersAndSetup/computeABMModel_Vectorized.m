...
function [walker_positions, walker_activation, C, DCLingerTime] = computeABMModel_Vectorized(walker_positions, walker_activation, C, DCLingerTime, DC_model, params)
    Ic = DC_model.Ic;
    ...
    BoundaryDC = DC_model.BoundaryDC;
        ...
        prob_up   = states_can_change .* near_boundary .*     (params.P_A) .* ( 1 - (walker_activation+params.da)./params.activatedAge);
        prob_down = states_can_change .* (1-near_boundary) .* (params.P_D) .* ( (walker_activation-params.da)/params.activatedAge);
        ...
        DCLingerTime(walker_activation == 0, :) = 0;
