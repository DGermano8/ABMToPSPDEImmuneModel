...
void computeABMModel_c(
    ...
    for (mwSize ii = 0; ii < num_walkers; ++ii) {
        ...
        /* --- 1. Random Motion --- */
        {
            ...
            double prob_left  = MAX(0.0,  (p_move / 4.0));
            double prob_right = MAX(0.0,  (p_move / 4.0));
            double prob_down  = MAX(0.0,  (p_move / 4.0));
            double prob_up    = MAX(0.0,  (p_move / 4.0));
        ...
        /* --- 2. Chemotaxis Motion --- */
        {
            ...
            double prob_left  = MAX(0.0,  (C[left_idx] - C_Here) * (C_chi / 4.0));
            double prob_right = MAX(0.0,  (C[right_idx] - C_Here) * (C_chi / 4.0));
            double prob_down  = MAX(0.0,  (C[down_idx] - C_Here) * (C_chi / 4.0));
            double prob_up    = MAX(0.0,  (C[up_idx] - C_Here) * (C_chi / 4.0));
            double prob_stay  = MAX(0.0, 1.0 - (prob_left + prob_right + prob_down + prob_up));
        ...
        /* --- 4. Update Activation --- */
        {
            ...
            double prob_up = (states_can_change && near_boundary) ? (P_A * (1.0 - walker_activation[ii] / activatedAge)) : 0.0;
            double prob_down = (states_can_change && !near_boundary) ? (P_D * (walker_activation[ii] / activatedAge)) : 0.0;
            
            if (near_boundary) {
                /* Compute the correct linear index for column-major order */
                mwSize dcl_index = (i_DC_id - 1) + (ii * NumberOfDCs); /* MATLAB-style indexing: row + num_rows * (col - 1) */
