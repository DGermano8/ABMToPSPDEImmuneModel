...
function [walker_positions, walker_activation, C, DCLingerTime] = computeSimpleABMModel_Vectorized(walker_positions, walker_activation, C, DCLingerTime, DC_model, params)
    Ic = DC_model.Ic;
    ...
    BoundaryDC = DC_model.BoundaryDC;
        ...
        % left_indices  = sub2ind([params.Nx, params.Ny], id_y, id_x_m1);
        % right_indices = sub2ind([params.Nx, params.Ny], id_y, id_x_p1);
        % down_indices  = sub2ind([params.Nx, params.Ny], id_y_m1, id_x);
        % up_indices    = sub2ind([params.Nx, params.Ny], id_y_p1, id_x);
        left_indices  = id_y + params.Nx.*(id_x_m1-1);
        right_indices = id_y + params.Nx.*(id_x_p1-1);
        down_indices  = id_y_m1 + params.Nx.*(id_x-1);
        up_indices    = id_y_p1 + params.Nx.*(id_x-1);

        % Compute movement probabilities
        prob_left =  max(0, (params.p_move/4) );
        prob_right = max(0, (params.p_move/4) );
        prob_down =  max(0, (params.p_move/4) );
        prob_up =    max(0, (params.p_move/4) );
        ...
        % here_indices  = sub2ind([params.Nx, params.Ny], id_y, id_x);
        % left_indices  = sub2ind([params.Nx, params.Ny], id_y, id_x_m1);
        % right_indices = sub2ind([params.Nx, params.Ny], id_y, id_x_p1);
        % down_indices  = sub2ind([params.Nx, params.Ny], id_y_m1, id_x);
        % up_indices    = sub2ind([params.Nx, params.Ny], id_y_p1, id_x);

        here_indices  = id_y + params.Nx.*(id_x-1);
        left_indices  = id_y + params.Nx.*(id_x_m1-1);
        right_indices = id_y + params.Nx.*(id_x_p1-1);
        down_indices  = id_y_m1 + params.Nx.*(id_x-1);
        up_indices    = id_y_p1 + params.Nx.*(id_x-1);
        ...
        % Compute movement probabilities
        prob_left =  max(0, (C_Left-C_Here) .*(params.C_chi/4) );
        prob_right = max(0, (C_Right-C_Here).*(params.C_chi/4) );
        prob_down =  max(0, (C_Down-C_Here) .*(params.C_chi/4) );
        prob_up =    max(0, (C_Up-C_Here)   .*(params.C_chi/4) );
        ...
        % prob_up = states_can_change .* near_boundary .*       (params.P_A/params.da) * params.dt .* ( 1 - walker_activation./params.activatedAge);
        % prob_down = states_can_change .* (1-near_boundary) .* (params.P_D/params.da) * params.dt .* walker_activation/params.activatedAge;
        prob_up = states_can_change .* near_boundary .*       (params.P_A) .* ( 1 - walker_activation./params.activatedAge);
        prob_down = states_can_change .* (1-near_boundary) .* (params.P_D) .* ( walker_activation/params.activatedAge);
        ...
        here_indices = near_boundary_id + size(DCLingerTime,1) .* (i_DC_id(near_boundary_id)-1);
        DCLingerTime(here_indices) = DCLingerTime(here_indices) + params.dt;
        
        walker_activation(Rad < prob_up & Rad > 0) =  walker_activation(Rad < prob_up & Rad > 0) + params.da;
        walker_activation(Rad < prob_up + prob_down & Rad > prob_up) =  walker_activation(Rad < prob_up + prob_down & Rad > prob_up) - params.da;
