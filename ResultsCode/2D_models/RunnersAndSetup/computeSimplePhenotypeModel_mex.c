 ...
 *   11: da     : double scalar
 *   12: dt     : double scalar
 *   13: dims   : int32 [3] array {nx,ny,na}
 *
 * Output:
...
{
    // if(nrhs!=14 || nlhs!=1) mexErrMsgIdAndTxt("pde_step_masked_opt_mex:nargs","Usage: u_new = pde_step_masked_opt_mex(u,C,A,A_Ic,chix,Dx,dx,dy,rho_p,rho_m,amax,da,dt,i_e,i_w,i_n,i_s,i_c,dims);");
    ...
    int32_T *dims_int = (int32_T*)mxGetData(prhs[13]);
    ...
    for(mwSize k=0; k<na; ++k) 
    {
        for(mwSize j=0; j<ny; ++j) 
        {
            for(mwSize i=0; i<nx; ++i) 
            {
                ...
                u_new[idx] = u_c + dt * (grad_x + grad_y + grad_a);
