...
function [walker_positions, walker_activation, C, DCLingerTime] = computeABMModel_Vectorized(walker_positions, walker_activation, C, DCLingerTime, DC_model, params)
    ...
    for ii = 1:(params.num_walkers)
        % Get the walker's current position
        wx = walker_positions(ii, 1);
        wy = walker_positions(ii, 2);
        ...
        if( Rad < p1 & Rad > 0 )
            id_x = id_x_m1;
        elseif( Rad < p2 & Rad > p1 )
            id_x = id_x_p1;
        elseif( Rad < p3 & Rad > p2 )
            id_y = id_y_m1;
        elseif ( Rad < p4 & Rad > p3 )
            id_y = id_y_p1;
        end
        ...
        walker_positions(ii,1) = params.dx*(id_x-1);
        walker_positions(ii,2) = params.dy*(id_y-1);

        %% Update the activation level
        states_can_change = ( walker_activation(ii) < params.activatedAge);
        ...
        near_boundary_id = (i_DC_id > 0 & states_can_change > 0);
        near_boundary = (i_DC_id > 0);
        % Compute probabilities
        prob_up = states_can_change .* near_boundary .*       (params.P_A) .* ( 1 - walker_activation(ii)./params.activatedAge);
        prob_down = states_can_change .* (1-near_boundary) .* (params.P_D) .* ( walker_activation(ii)/params.activatedAge);
        ...
        if ( Rad < prob_up & Rad > 0 )
            walker_activation(ii) =  walker_activation(ii) + params.da;
        elseif ( Rad < prob_up + prob_down & Rad > prob_up )
            walker_activation(ii) =  walker_activation(ii) - params.da;
        end
