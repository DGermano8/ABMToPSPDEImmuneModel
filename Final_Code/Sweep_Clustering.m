...
% T-cells 
% Low
% ModelParams.P_A = 0.80; % Antigen gain rate
% High
ModelParams.P_A = 0.95; % Antigen gain rate

ModelParams.P_D = 0.10;  % Antigen loss rate
...
number_of_clusters = [16 8 4 2 1];

% Activation_Cluster_ABM = zeros(length(number_of_clusters), numb_rands);
Activation_Cluster_PDE = zeros(length(number_of_clusters), numb_rands);

% FullActivated_Cluster_ABM = zeros(length(number_of_clusters), numb_rands);
FullActivated_Cluster_PDE = zeros(length(number_of_clusters), numb_rands);
...
tic;
for cluster_i = 1:length(number_of_clusters)

    ModelParams.numberOfClusters = number_of_clusters(cluster_i);
    ...
    parfor jj=1:numb_rands
        ...
        Activation_Cluster_ABM(cluster_i,jj) = activatedCells_ABM(end);
        FullActivated_Cluster_ABM(cluster_i,jj) = activatedCells_T_ABM(end);
        ...
        % end
        Activation_Cluster_PDE(cluster_i,jj) = activatedCells_PDE(end);
        FullActivated_Cluster_PDE(cluster_i,jj) = activatedCells_T_PDE(end);
        ...
        [number_of_clusters(cluster_i) jj]

    end
    iter = iter + numb_rands;
    prog = (iter)/(numb_rands*length(number_of_clusters));
...
if ModelParams.P_A < 0.5
    plot_colour = red_colour;
else
    plot_colour = blue_colour;
end
...
violinplot((Activation_Cluster_PDE/ModelParams.activatedAge)', number_of_clusters, ...
    'showdata',false ,'HalfViolin','right',  'ViolinColor',plot_colour, 'ViolinAlpha', 0.75);
xticklabels(["1"; "2"; "4"; "8"; "16"]')
xlabel('Size of cluster (Numb. DCs)', 'FontSize',12, 'Interpreter','latex');
...
violinplot((FullActivated_Cluster_PDE/ModelParams.activatedAge)', number_of_clusters, ...
    'showdata',false,'HalfViolin','right',  'ViolinColor',plot_colour, 'ViolinAlpha', 0.75);
xticklabels(["1"; "2"; "4"; "8"; "16"]')
xlabel('Size of cluster (Numb. DCs)', 'FontSize',12, 'Interpreter','latex');
...
violinplot(Activation_Cluster_ABM', number_of_clusters, ...
    'showdata',false ,'HalfViolin','right',  'ViolinColor',plot_colour, 'ViolinAlpha', 0.75);
xticklabels(["1"; "2"; "4"; "8"; "16"]')
xlabel('Size of cluster (Numb. DCs)', 'FontSize',12, 'Interpreter','latex');
...
violinplot(FullActivated_Cluster_ABM', number_of_clusters, ...
    'showdata',false,'HalfViolin','right',  'ViolinColor',plot_colour, 'ViolinAlpha', 0.75);
xticklabels(["1"; "2"; "4"; "8"; "16"]')
xlabel('Size of cluster (Numb. DCs)', 'FontSize',12, 'Interpreter','latex');
title('Proportion of activated T cells', 'FontSize',14, 'Interpreter','latex');
