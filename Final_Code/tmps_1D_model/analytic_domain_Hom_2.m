...
for ii = 2:n_max
   ...
   % Draw a random event
    ...
    % delay_time = 0;
    % if gain_rate > 0
    %     delay_time = delay_time + 2*max_A/(gain_rate);
    % end
    % if loss_rate > 0
    %     delay_time = delay_time + 2*max_A/(loss_rate);
    % end
    % final_state_change = gain_rate/(max_A^1.75)*Ic.*(A_old)  * (ii*dt > delay_time);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % x_max = 5
    % final_state_change = gain_rate/(max_A^1)*Ic.*(A_old)  * (ii*dt > delay_time);
    % final_state_change = gain_rate/(max_A^1)*Ic.*(A_old).*U_old  * (ii*dt > delay_time);

    % hill_ord = 2;
    % final_state_change = gain_rate/(max_A).*Ic.*A_old.*(A_old.^hill_ord./((U_old).^hill_ord+A_old.^hill_ord+1e-10));
    
    % final_state_change = gain_rate/(max_A^act_pow)*Ic.*(A_old)  * (ii*dt > delay_time);
    ...
    % final_state_change = gain_rate/(max_A^2)*Ic.*(A_old);

    % final_state_change = (1/(max_A*loss_rate))*(gain_rate)*Ic.*(A_old/(max_A)).*(A_old/(max_A-1) > U_old);
    % final_state_change = (gain_rate)*Ic.*(A_old/(max_A)).*(A_old + A_T_old > U_old*(0.75*max_A));
    % final_state_change = (gain_rate)*Ic.*(A_old/(max_A^1)).* ((A_old + A_T_old)/max_A).^2./ ((U_old).^2 + ((A_old + A_T_old)/max_A).^2 + 1e-10);
    % final_state_change = (gain_rate)*Ic.*(A_old/(max_A^1)).* ((A_old )).^2./ ((U_old*max_A).^2 + ((A_old )).^2 + 1e-10);

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    final_state_change = 0;
    if loss_rate > 0
        final_state_change = (gain_rate) .* (1./(max_A^2)) .* A_old.*Ic .* (  A_old +A_T_old > (U_old*(max_A-1)*(loss_rate)/(gain_rate)*sum(U_old(Ic==1))) );
    else
        final_state_change = (gain_rate) .* (1./(max_A^1)) .* A_old.*Ic .* (  A_old +A_T_old > U_old*(max_A-1) );
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    final_state_change = 0;
    % if loss_rate > 0
    %     final_state_change = (gain_rate) .* (1./(max_A^2)) .* (A_old.^2).*Ic;
    % else
    %     final_state_change = (gain_rate) .* (1./(max_A^1)) .* A_old.*Ic; 
    % end

    final_state_change = 0;
    % final_state_change = A_old.*Ic.*(max_A)*(gamma(1+1/loss_rate)*gamma(1+1/gain_rate)/gamma(2+1/loss_rate+1/gain_rate) - betainc((max_A-1)/max_A,1+1/loss_rate,1+1/gain_rate));
    ...
    if ismember(ii,plot_times) %10/dt) == 0
        ...
        if false
            ...
            % % plot(Ic, '--')
            % axis([0 x_max 0 0.05])
            axis([0 x_max 0 max((A_T_pde+A_pde)/max_A)*1.1 ])
        ...
        if true
            ...
            % plot(Ic, '--')
            % axis([0 nx 0 0.05])
            axis([0 nx 0 max((A_T_pde+A_pde)/max_A)*1.1 ])
