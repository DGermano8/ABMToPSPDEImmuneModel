...
for ii = 2:n_max
   ...
   % Draw a random event
    ...
    % 
    % 
    A_pde = A_old + dt * ( D_move * (axx) - chi_sense*a_taxis + gain_rate/max_A*Ic.*U_old.*(1-(A_old+A_T_old))  - loss_rate/max_A.*(1-Ic).*A_old );%
    ...
    activation_T_pde(ii) = sum(A_T_pde);
    ...
    if ismember(ii,plot_times) %10/dt) == 0
        ...
        if true
            ...
            plot((A_T_pde+A_pde)*dx, '--','LineWidth',2)
