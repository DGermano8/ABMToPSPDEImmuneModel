...
plot3(0:dx:x_max, 0*ones(length(0:dx:x_max),1), A_T_pde+sum(A_pde,2), '--','LineWidth',pde_line,   'Color', red_colour)
...
for ii = 2:n_max
   ...
   % Draw a random event
    ...
    a_extended = padarray(sum(A_pde,2) + A_T_pde , [1], 'replicate'); % Extend boundaries by mirroring
    a_shift_xp = a_extended(3:end); % Shift down
    a_shift_xm = a_extended(1:end-2); % Shift up
    axx = (a_shift_xp - 2 * (sum(A_pde,2) + A_T_pde ) + a_shift_xm) / (dx)^2;

     % For du_dx and du_dy terms
    da1_dx = ( (sum(A_pde,2) + A_T_pde )  + a_shift_xp) / 2;
    da2_dx = ( (sum(A_pde,2) + A_T_pde )  + a_shift_xm) / 2;
    % Taxis terms
    a_taxis = (da1_dx .* dc1_dx - da2_dx .* dc2_dx) / dx;

    movement_dt = D_move * (axx) - chi_sense*a_taxis;
    ...
    for jj = 1:max_A
        ...
        if jj == max_A
            ...
            final_state_change = gain_rate/(max_A^1)*Ic.*(A_jj_old) * (ii*dt > delay_time);
            A_pde(:,jj) = A_jj_old + dt * ( movement_dt + gain_rate*Ic.*(U_old*(jj) .* 1/max_A)  - loss_rate/max_A.*(1-Ic).*A_jj_old - final_state_change );%
        
        elseif jj == 1
            A_pde(:,jj) = A_jj_old + dt * ( movement_dt + loss_rate/max_A.*(1-Ic).*A_old(:,jj+1));%

        else
            A_pde(:,jj) = A_jj_old + dt * ( movement_dt + gain_rate*Ic.*(U_old*(jj) .* 1/max_A)  - loss_rate/max_A.*(1-Ic).*A_jj_old) + loss_rate/max_A.*(1-Ic).*A_old(:,jj+1));%
    ...
    A_T_pde = A_T_old  + dt * ( movement_dt + final_state_change ) ;%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % 
    % 
    % final_state_change = gain_rate/(max_A^2.5)*Ic.*(A_old) * (ii*dt > delay_time);
    % 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    activation_pde(ii) = sum(A_T_pde+sum(A_pde,2));
    ...
    if ismember(ii,plot_times) %10/dt) == 0
        ...
        if true
            ...
            plot((A_T_pde+sum(A_pde,2)), '--','LineWidth',2)
            % plot((A_T_pde+sum(A_pde,2)).*U_pde, '--','LineWidth',2)
        ...
        if false
            ...
            plot3(0:dx:x_max, ii*dt*ones(length(0:dx:x_max),1), (A_T_pde+sum(A_pde,2)).*U_pde, '--','LineWidth',pde_line,   'Color', red_colour)
