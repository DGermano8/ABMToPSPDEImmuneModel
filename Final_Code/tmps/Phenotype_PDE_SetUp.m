function [U, C, Ic, A_Ic, Ic_vals, params, A] = Phenotype_PDE_SetUp(rndSeed,domainBoundary,ModelParams)
    ...
    U = zeros(Ny, Nx, Na);          % Population density u
    U(:,1,1) = 1;
    U=U/(sum(U(:)));
    ...
    % Initialize fields
    C = zeros(Ny, Nx, Na);          % Chemical concentration c
    ...
    if ModelParams.NumDCs > 0
        ...
        [Ic, A_Ic, Inds_Ic_tmp] =  getProximityField(Nx, Ny, Na,  [x_DC,y_DC], R_DC ,dx,dy, R_DC); % Proximity field
    else
        DCs = zeros(Ny, Nx);
        Ic = zeros(Ny, Nx);
    end
