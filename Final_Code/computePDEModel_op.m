...
function [U, A, U_a, A_a] = computePDEModel_op(U, A, U_a, A_a, Ic, Inds_Ic, A_Ic,  dc1_dx, dc2_dx, dc1_dy, dc2_dy,params)
    ...
    % uxx = (u_X_extended(3:end, 2:end-1) - 2 * U_X_old + u_X_extended(1:end-2, 2:end-1)) / (params.dx)^2;
    % uyy = (u_Y_extended(2:end-1, 3:end) - 2 * U_Y_old + u_Y_extended(2:end-1, 1:end-2)) / (params.dy)^2;
    diffusion = params.D * ((u_X_extended(3:end, 2:end-1) - 2 * U_X_old + u_X_extended(1:end-2, 2:end-1)) / (params.dx)^2 +...
                            (u_Y_extended(2:end-1, 3:end) - 2 * U_Y_old + u_Y_extended(2:end-1, 1:end-2)) / (params.dy)^2);
    
    % For du_dx and du_dy terms
    du1_dx = (U_X_old + u_X_extended(3:end, 2:end-1)) / 2;
    du2_dx = (U_X_old + u_X_extended(1:end-2, 2:end-1)) / 2;
    du1_dy = (U_Y_old + u_Y_extended(2:end-1, 3:end)) / 2;
    du2_dy = (U_Y_old + u_Y_extended(2:end-1, 1:end-2)) / 2;
    % Taxis terms
    % taxis_x = (du1_dx .* dc1_dx - du2_dx .* dc2_dx) / params.dx;
    % taxis_y = (du1_dy .* dc1_dy - du2_dy .* dc2_dy) / params.dy;
    taxis = params.C_chi * ((du1_dx .* dc1_dx - du2_dx .* dc2_dx) / params.dx +...
                            (du1_dy .* dc1_dy - du2_dy .* dc2_dy) / params.dy);
    ...
    % Central difference for diffusion
    % Axx = (A_X_extended(3:end, 2:end-1) - 2 * (A_X_old) + A_X_extended(1:end-2, 2:end-1)) / (params.dx)^2;
    % Ayy = (A_Y_extended(2:end-1, 3:end) - 2 * (A_Y_old) + A_Y_extended(2:end-1, 1:end-2)) / (params.dy)^2;
    A_diffusion = params.D * ((A_X_extended(3:end, 2:end-1) - 2 * (A_X_old) + A_X_extended(1:end-2, 2:end-1)) / (params.dx)^2 + ...
                              (A_Y_extended(2:end-1, 3:end) - 2 * (A_Y_old) + A_Y_extended(2:end-1, 1:end-2)) / (params.dy)^2);
    
    % For du_dx and du_dy terms
    dA1_dx = ((A_X_old) + A_X_extended(3:end, 2:end-1)) / 2;
    dA2_dx = ((A_X_old) + A_X_extended(1:end-2, 2:end-1)) / 2;
    dA1_dy = ((A_Y_old) + A_Y_extended(2:end-1, 3:end)) / 2;
    dA2_dy = ((A_Y_old) + A_Y_extended(2:end-1, 1:end-2)) / 2;
    % Taxis terms
    % taxis_x = (dA1_dx .* dc1_dx - dA2_dx .* dc2_dx) / params.dx;
    % taxis_y = (dA1_dy .* dc1_dy - dA2_dy .* dc2_dy) / params.dy;
    A_taxis = params.C_chi * ((dA1_dx .* dc1_dx - dA2_dx .* dc2_dx) / params.dx + ...
                              (dA1_dy .* dc1_dy - dA2_dy .* dc2_dy) / params.dy);
    ...
    A = A_old + params.dt * ( A_diffusion - A_taxis  + params.P_A .* A_Ic .* (1./(params.activatedAge)).*(U_old - (A_old+A_a_old) )  ...
                       ...
                       - params.dt .* params.P_A .* (1./(params.activatedAge^1.5)) .* A_old.*A_Ic;
    ...
    % Central difference for diffusion
    % Axx = (A_X_extended(3:end, 2:end-1) - 2 * (A_a_X_old) + A_X_extended(1:end-2, 2:end-1)) / (params.dx)^2;
    % Ayy = (A_Y_extended(2:end-1, 3:end) - 2 * (A_a_Y_old) + A_Y_extended(2:end-1, 1:end-2)) / (params.dy)^2;
    A_diffusion = params.D * ((A_X_extended(3:end, 2:end-1) - 2 * (A_a_X_old) + A_X_extended(1:end-2, 2:end-1)) / (params.dx)^2 + ...
                              (A_Y_extended(2:end-1, 3:end) - 2 * (A_a_Y_old) + A_Y_extended(2:end-1, 1:end-2)) / (params.dy)^2);
    
    % For du_dx and du_dy terms
    dA1_dx = ((A_a_X_old) + A_X_extended(3:end, 2:end-1)) / 2;
    dA2_dx = ((A_a_X_old) + A_X_extended(1:end-2, 2:end-1)) / 2;
    dA1_dy = ((A_a_Y_old) + A_Y_extended(2:end-1, 3:end)) / 2;
    dA2_dy = ((A_a_Y_old) + A_Y_extended(2:end-1, 1:end-2)) / 2;
    % Taxis terms
    % taxis_x = (dA1_dx .* dc1_dx - dA2_dx .* dc2_dx) / params.dx;
    % taxis_y = (dA1_dy .* dc1_dy - dA2_dy .* dc2_dy) / params.dy;
    A_taxis = params.C_chi * ((dA1_dx .* dc1_dx - dA2_dx .* dc2_dx) / params.dx + ...
    ...
    A_a = A_a_old + params.dt .* params.P_A .* (1./(params.activatedAge^1.5)) .* A_old.*A_Ic + params.dt * ( A_diffusion - A_taxis) ;
